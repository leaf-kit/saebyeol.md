...
| Linux x86_64 | `saebyeol-<version>-1.x86_64.rpm` | Fedora / RHEL / openSUSE dnf · zypper 호환 |
| Linux x86_64 | `saebyeol_<version>_amd64.AppImage` | 배포판 무관, 실행 권한만 부여하면 됨 |
| Windows x86_64 | `saebyeol_<version>_x64-setup.exe` | NSIS 설치 마법사. (MSI 는 productName "새별" 비-ASCII 출력에서 WiX `light.exe` 가 실패하는 이슈로 빌드에서 제외) |
